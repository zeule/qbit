...
namespace
{
    ...
    SplittedValue getRoundedYScale(double value)
    {
        ...
        if (value < 0.5) return {0, SizeUnit::Byte};
...
SpeedPlotView::Averager::Averager(unsigned int divider, boost::circular_buffer<PointData> &sink)
    : m_divider(divider)
    , m_sink(sink)
    , m_counter(0)
    , m_accumulator {}
...
quint64 SpeedPlotView::maxYValue()
{
    ...
    for (int id = UP; id < NB_GRAPHS; ++id) {
        ...
        for (int i = static_cast<int>(queue.size()) - 1, j = 0; (i >= 0) && (j <= m_viewablePointsCount); --i, ++j)
            if (queue[static_cast<std::size_t>(i)].y[id] > maxYValue)
                maxYValue = queue[static_cast<std::size_t>(i)].y[id];
    }
...
void SpeedPlotView::paintEvent(QPaintEvent *)
{
    ...
    const double yMultiplier = (niceScale.arg < 0.5) ? 0.0 : (static_cast<double>(rect.height()) / niceScale.sizeInBytes());
    ...
    for (int id = UP; id < NB_GRAPHS; ++id) {
        ...
        for (int i = static_cast<int>(queue.size()) - 1, j = 0; (i >= 0) && (j <= m_viewablePointsCount); --i, ++j) {
            ...
            int newY = rect.bottom() - queue[static_cast<std::size_t>(i)].y[id] * yMultiplier;
