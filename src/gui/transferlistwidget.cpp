...
void TransferListWidget::setMaxRatioSelectedTorrents()
{
    ...
    std::chrono::minutes currentMaxSeedingTime = BitTorrent::Session::instance()->globalMaxSeedingMinutes();
    if (torrents.count() == 1)
...
void TransferListWidget::displayListMenu(const QPoint&)
{
    ...
    connect(&actionIncreasePriority, &QAction::triggered, this, &TransferListWidget::increaseQueuePosSelectedTorrents);
    QAction actionDecreasePriority(GuiIconProvider::instance()->getIcon("go-down"), tr("Move down", "i.e. Move down in the queue"), nullptr);
    connect(&actionDecreasePriority, &QAction::triggered, this, &TransferListWidget::decreaseQueuePosSelectedTorrents);
    QAction actionTopPriority(GuiIconProvider::instance()->getIcon("go-top"), tr("Move to top", "i.e. Move to top of the queue"), nullptr);
    connect(&actionTopPriority, &QAction::triggered, this, &TransferListWidget::topQueuePosSelectedTorrents);
    ...
    connect(&actionBottomPriority, &QAction::triggered, this, &TransferListWidget::bottomQueuePosSelectedTorrents);
