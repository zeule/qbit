...
QString TransferListModel::displayValue(const BitTorrent::TorrentHandle *torrent, const int column) const
{
    ...
    const auto lastActivityString = [hideValues](std::chrono::seconds value) -> QString
    {
        if (hideValues && ((value < value.zero()) || (value >= MAX_ETA)))
            return QString {};
    ...
    const auto timeElapsedString = [](const std::chrono::seconds elapsedTime, const std::chrono::seconds seedingTime) -> QString
    {
        if (seedingTime <= seedingTime.zero())
    ...
    case TR_LAST_ACTIVITY:
        return lastActivityString((torrent->isPaused() || torrent->isChecking()) ? std::chrono::seconds(-1) : torrent->timeSinceActivity());
    case TR_AVAILABILITY:
...
QVariant TransferListModel::internalValue(const BitTorrent::TorrentHandle *torrent, const int column, const bool alt) const
{
    ...
    case TR_TIME_ELAPSED:
        return QVariant::fromValue(!alt ? torrent->activeTime() : torrent->seedingTime());
    case TR_SAVE_PATH:
        return Utils::Fs::toNativePath(torrent->savePath());
    case TR_COMPLETED:
    ...
    case TR_LAST_ACTIVITY:
        return QVariant::fromValue((torrent->isPaused() || torrent->isChecking()) ? std::chrono::seconds(-1) : torrent->timeSinceActivity());
    case TR_AVAILABILITY:
