...
void AppController::preferencesAction()
{
    ...
    // Connection
    // Listening Port
    data["listen_port"] = static_cast<int>(session->port());
    ...
    data["max_seeding_time_enabled"] = (session->globalMaxSeedingMinutes().count() >= 0);
    data["max_seeding_time"] = static_cast<int>(session->globalMaxSeedingMinutes().count());
...
void AppController::setPreferencesAction()
{
    ...
    if (hasKey("listen_port"))
        session->setPort(it.value().toUInt());
    if (hasKey("upnp"))
    ...
    if (hasKey("max_seeding_time_enabled")) {
        if (it.value().toBool())
            session->setGlobalMaxSeedingMinutes(std::chrono::minutes(m["max_seeding_time"].toInt()));
        else
            session->setGlobalMaxSeedingMinutes(std::chrono::minutes(-1));
    }
    ...
    if (hasKey("save_resume_data_interval"))
        session->setSaveResumeDataInterval(it.value().toUInt());
    // Recheck completed torrents
