...
bool GeoIPDatabase::parseMetadata(const QVariantHash &metadata, QString &error)
{
    ...
    m_buildEpoch = QDateTime::fromSecsSinceEpoch(metadata.value("build_epoch").toLongLong());
...
QVariantHash GeoIPDatabase::readMetadata() const
{
    ...
    if (index >= 0) {
        ...
        auto offset = static_cast<quint32>(index) + static_cast<quint32>(strlen(METADATA_BEGIN_MARK));
